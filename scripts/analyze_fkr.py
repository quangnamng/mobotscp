...
if __name__ == "__main__":
  ...
  gen_fkr_param = fkreach.GenerateFKR(base_approach_dir=[1., 0], sampling_mode="visible-front", \
                                      xyz_delta=0.04, angle_inc=np.pi/12, angle_offset=-np.pi/4, \
                                      max_radius=None, orientation_list=orien_list, l1_from_ground=0.5175)
  # > comment the line below to generate new FKR
  gen_fkr_param = None

  # Generate new or load available FKR data
  ...
  # Visualize FKR iso surface & reachability limits
